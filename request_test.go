...
func TestUnmarshall_attrStringSlice(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(bytes.NewReader([]byte(data)), out); err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalPayloadWithPointerID(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(sampleWithPointerPayload(attrs), out); err != nil {
...
func TestUnmarshalPayloadWithPointerAttr_AbsentVal(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(sampleWithPointerPayload(in), out); err != nil {
		t.Fatalf("Error unmarshalling to Foo")
	}
...
func TestUnmarshalToStructWithPointerAttr_BadType_bool(t *testing.T) {
	...
	err := jsonapi.UnmarshalPayload(sampleWithPointerPayload(in), out)
	...
	if _, ok := err.(jsonapi.ErrUnsupportedPtrType); !ok {
...
func TestStringPointerField(t *testing.T) {
	...
	book := new(Book)
	if err := jsonapi.UnmarshalPayload(bytes.NewReader(payload), book); err != nil {
		t.Fatal(err)
	}
...
func TestMalformedTag(t *testing.T) {
	out := new(BadModel)
	err := jsonapi.UnmarshalPayload(samplePayload(), out)
	if err == nil || err != jsonapi.ErrBadJSONAPIStructTag {
...
func TestUnmarshalInvalidJSON(t *testing.T) {
	...
	err := jsonapi.UnmarshalPayload(in, out)
...
func TestUnmarshal_nonNumericID(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(in, out); err != jsonapi.ErrBadJSONAPIID {
		t.Fatalf(
			"Was expecting a `%s` error, got `%s`",
			jsonapi.ErrBadJSONAPIID,
			err,
		)
	}
}
...
func TestUnmarshalInvalidISO8601(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(in, out); err != jsonapi.ErrInvalidISO8601 {
...
func TestUnmarshalNestedRelationshipsEmbedded(t *testing.T) {
	out := bytes.NewBuffer(nil)
	if err := jsonapi.MarshalOnePayloadEmbedded(out, testModel()); err != nil {
		t.Fatal(err)
	}

	model := new(Blog)

	if err := jsonapi.UnmarshalPayload(out, model); err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalRelationshipsSideloaded(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(payload, out); err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalManyPayload(t *testing.T) {
	...
	posts, err := jsonapi.UnmarshalManyPayload(in, reflect.TypeOf(new(Post)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestManyPayload_withLinks(t *testing.T) {
	...
	sample := map[string]interface{}{
		...
		"links": map[string]interface{}{
			jsonapi.KeyFirstPage:    firstPageURL,
			jsonapi.KeyPreviousPage: prevPageURL,
			jsonapi.KeyNextPage:     nextPageURL,
			jsonapi.KeyLastPage:     lastPageURL,
		},
	}
	...
	first, ok := links[jsonapi.KeyFirstPage]
	if !ok {
		t.Fatal("Was expecting a non nil ptr Link field")
	}
	if e, a := firstPageURL, first; e != a {
		t.Fatalf("Was expecting links.%s to have a value of %s, got %s", jsonapi.KeyFirstPage, e, a)
	}

	prev, ok := links[jsonapi.KeyPreviousPage]
	if !ok {
		t.Fatal("Was expecting a non nil ptr Link field")
	}
	if e, a := prevPageURL, prev; e != a {
		t.Fatalf("Was expecting links.%s to have a value of %s, got %s", jsonapi.KeyPreviousPage, e, a)
	}

	next, ok := links[jsonapi.KeyNextPage]
	if !ok {
		t.Fatal("Was expecting a non nil ptr Link field")
	}
	if e, a := nextPageURL, next; e != a {
		t.Fatalf("Was expecting links.%s to have a value of %s, got %s", jsonapi.KeyNextPage, e, a)
	}

	last, ok := links[jsonapi.KeyLastPage]
	if !ok {
		t.Fatal("Was expecting a non nil ptr Link field")
	}
	if e, a := lastPageURL, last; e != a {
		t.Fatalf("Was expecting links.%s to have a value of %s, got %s", jsonapi.KeyLastPage, e, a)
	}
}
...
func TestUnmarshalCustomTypeAttributes(t *testing.T) {
	...
	if err := jsonapi.UnmarshalPayload(bytes.NewReader(payload), customAttributeTypes); err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalCustomTypeAttributes_ErrInvalidType(t *testing.T) {
	...
	// Parse JSON API payload
	customAttributeTypes := new(CustomAttributeTypes)
	err = jsonapi.UnmarshalPayload(bytes.NewReader(payload), customAttributeTypes)
	if err == nil {
